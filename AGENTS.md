...
- ChLens Liveが開発中の現在は、通常のIssue調査、重複確認、関連コードの照合には`feature/chlens-live`を参照する。Issue本文やコメントの内容だけで実装済みと判断せず、このブランチの最新コードで確認する。
- `develop`は安定版およびLiveの昇格先として扱い、安定版との差分確認や、Issueが明示的に安定版を対象としている場合に参照する。通常のChLens Live関連Issueの調査結果を`develop`だけに基づいて記録しない。
- `ai/*`などの作業ブランチ、個別の過去PRブランチ、ローカルだけに存在するブランチは、統合済みの最新状態を示す参照元として扱わない。必要な実装の所在を確認する場合に限り、補助的に参照する。
- ChLens Liveが`develop`へ昇格した後は、この節と`ChLens Live branch policy`の参照先を、昇格後の運用に合わせて更新する。
