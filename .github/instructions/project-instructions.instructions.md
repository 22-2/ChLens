...
- トリアージAIが読むtodoは、リポジトリ直下の`.todo`だけです。
- `apps/chlens-live/todo.md`はLive固有の設計・作業メモであり、トリアージ入力や進捗の正本ではありません。
- GitHub Issueを実装タスクと進捗の正本にし、PRはレビューと取り込みの窓口として扱ってください。
- `develop`を最新コードの参照先とし、作業ブランチとPRのbaseも`develop`にしてください。
