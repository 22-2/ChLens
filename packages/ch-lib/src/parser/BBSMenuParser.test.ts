...
describe('BBSMenuParser', () => {
  it('should parse BBS menu HTML', () => {
    const html = `
      ...
      <A HREF=http://board1.5ch.net/test/>Board 1</A><BR>
      <A HREF=http://board2.5ch.net/test/>Board 2</A><BR>
      <BR><BR><B>Category 2</B><BR>
      <A HREF=http://board3.5ch.net/test/>Board 3</A><BR>
    `;
    ...
    expect(result[0].boards[0].url).toBe('http://board1.5ch.net/test/');
