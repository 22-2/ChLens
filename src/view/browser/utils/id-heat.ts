...
export function getIdHeatColor(idCount: number): string {
  if (idCount <= 1) {
    return "var(--sys-color-res-id-muted)";
  }
  ...
  if (idCount <= ID_HEAT_COOL_PEAK_COUNT) {
    ...
    return `color-mix(in srgb, var(--sys-color-res-id-muted) ${mutedPercent}%, var(--sys-color-res-id-cool) ${coolPercent}%)`;
  }
  ...
  return `color-mix(in srgb, var(--sys-color-res-id-cool) ${coolPercent}%, var(--sys-color-res-heat-hot) ${hotPercent}%)`;
}
