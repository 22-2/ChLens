...
app.boot("/view/thread.html", async function () {
  ...
  $view.on("request_reload", async function ({ detail: ex = {} }) {
    ...
    const jumpResNum = +((left =
      ex.written_res_num != null ? ex.written_res_num : ex.param_res_num) !=
    null
      ? left
      : -1);
    if (
    ...
    const thread = await app.viewThread._draw($view, {
    ...
    for (let i = thread.res.length - 1; i >= 0; i--) {
      ...
      if (
        postMes ===
        app.util
          .decodeCharReference(app.util.stripTags(t.message))
          .replace(/\s/g, "")
      ) {
  ...
  (async function () {
    ...
    $view.on(
      "read_state_attached",
      function ({ detail: { jumpResNum, requestReloadFlag, loadCount } = {} }) {
        let defaultScroll;
        let onScroll = false;
        $content.on(
          "scroll",
        ...
        (defaultScroll = function () {
          ...
          if (0 < jumpResNum && jumpResNum <= lastNum) {
            ...
            let left;
            const offset =
              (left = $last.attr("last-offset")) != null ? left : 0;
        ...
        $view.on(
          ...
          function ({
            ...
            if (
              $view.hasClass("autoload") &&
              !$view.hasClass("autoload_pause")
            ) {
              ...
              case "new":
                ...
                if (0 < jumpResNum && jumpResNum <= lastNum) {
                  ...
                  if ($tmp == null) {
                    ...
                    offset =
                      (left =
                        $tmp != null ? $tmp.attr("last-offset") : undefined) !=
                      null
                        ? left
                        : -100;
                  }
...
`,
  ...
  $view.on("click", function (e) {
    ...
    const targetResNum =
      tmp.segments[0] != null ? tmp.segments[0][0] : undefined;
    ...
    function (e) {
      ...
      popupHelper(target, e, () => {
        ...
        for (let targetResNum of threadContent.repIndex.get(resNum)) {
          ...
          if (
            targetRes.hasClass("ng") &&
            (!targetRes.hasClass("disp_ng") || app.config.isOn("reject_ng_rep"))
          ) {
            continue;
          }
  ...
  $view.on("contextmenu", function ({ target }) {
    ...
    // リンクアドレスをNG登録
    let enableFlg = !(
      target.hasClass("anchor") || target.hasClass("anchor_id")
    );
    ...
    enableFlg =
      target.hasClass("open_in_rcrx") &&
      target.dataset.paramResNum !== undefined;
    ...
    if (["id", "slip"].includes(type)) {
      switch (app.config.get(`ng_${type}_expire`)) {
        case "date":
          var d =
            Date.now() +
            +app.config.get(`ng_${type}_expire_date`) * 86400 * 1000;
          exDate = new Date(d);
          break;
        case "day":
          var t = new Date();
  ...
  (function () {
    ...
    $searchbox.on(
      "input",
      function ({ isComposing, detail: { isEnter = false } = {} }) {
        let dom, scrollTop;
        if (isComposing) {
          return;
        }
  ...
  (function () {
    ...
    const $nextUnread = {
      ...
      show() {
        ...
        const bookmarks = app.bookmark
          .getAll()
          .filter(({ type, url }) => type === "thread" && url !== viewUrlStr);
        ...
        if (next) {
          ...
          if (next.resCount != null) {
            text += ` (未読${
              next.resCount -
              ((next.readState != null ? next.readState.read : undefined) || 0)
            }件)`;
          }
...
app.viewThread._readStateManager = async function ($view) {
  ...
  $view.on(
    "view_loaded",
    async function ({ detail: { jumpResNum, loadCount } }) {
