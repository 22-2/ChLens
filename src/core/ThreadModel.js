...
export default class ThreadModel {
  ...
  addRes(resRaw) {
    ...
    const { bbsType } = container.util.guessType ? container.util.guessType(this.urlStr) : { bbsType: "2ch" };
    ...
    // Over 1000 check
    // @ts-ignore
    if (bbsType === "2ch" && res.other && res.other.startsWith("Over 1000") && !this.over1000ResNum) {
  ...
  checkNG(res, bbsType) {
    ...
    if (bbsType === "2ch") {
      ...
      // Nothing ID
      if (container.config.get("nothing_id_ng") && !res.id &&
          ((judgeType === "first_res" && this._existIdAtFirstRes) || (judgeType === "exists_once" && this.idIndex.size > 0))) {
        if (!container.ng.isIgnoreResNumForAuto(res.num, "NothingID") &&
            !container.ng.isThreadIgnoreNgType(res, this.title, this.urlStr, "NothingID")) {
          return { type: "NothingID" };
        }
      }
      // Nothing Slip
      if (container.config.get("nothing_slip_ng") && !res.slip &&
          ((judgeType === "first_res" && this._existSlipAtFirstRes) || (judgeType === "exists_once" && this.slipIndex.size > 0))) {
        if (!container.ng.isIgnoreResNumForAuto(res.num, "NothingSLIP") &&
    ...
    if (container.config.get("chain_ng_id") && res.id && this._ngIdForChain.has(res.id)) {
      if (!container.ng.isIgnoreResNumForAuto(res.num, "ChainID") &&
    ...
    if (container.config.get("chain_ng_slip") && res.slip && this._ngSlipForChain.has(res.slip)) {
      if (!container.ng.isIgnoreResNumForAuto(res.num, "ChainSLIP") &&
    ...
    if (repeatCount > 1) {
       ...
       if (rset) {
           ...
           if (rset.size >= repeatCount) {
             if (!container.ng.isIgnoreResNumForAuto(res.num, "RepeatMessage") &&
  ...
  _handleNgDependencies(res, ngObj) {
    ...
    if (container.config.get("chain_ng_slip") && res.slip && !["Slip", "ChainSLIP"].includes(ngObj.type)) {
