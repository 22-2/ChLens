...
const openDB = () =>
  new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = ({
      ...
      if (oldVer < 1) {
        const objStore = db.createObjectStore("History", {
          keyPath: "id",
          autoIncrement: true,
        });
...
export var getUnique = function (offset, limit) {
  ...
  if (
    app.assertArg("History.getUnique", [
      [offset, "number"],
      [limit, "number"],
    ])
  ) {
